...
void uaTokenizerTest::testStaticTokenizeBreaksUpUserAgentIntoTokens()
{
    ...
    CPPUNIT_ASSERT_MESSAGE("expected Tokens.size()=7, actual=" + std::to_string(tokens.size()), 7 == tokens.size());
    ...
    CPPUNIT_ASSERT_MESSAGE("", "rv:44.0)" == tokens[4]);
    CPPUNIT_ASSERT_MESSAGE("", "Gecko/20100101" == tokens[5]);
    CPPUNIT_ASSERT_MESSAGE("", "Firefox/44.0" == tokens[6]);
}
